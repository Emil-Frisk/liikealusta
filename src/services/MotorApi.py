...
class MotorApi():
    ...
    async def rotate(self, pitch_value, roll_value):
        try:
            ...
            await self.write(address=self.ANALOG_MODBUS_CNTRL, left_val=position_client_left, right_val=position_client_right, different_values=True, description="Write modbustCNTRL MAX", )
            return True
        except Exception as e:
            self.logger.error(f"Something went wrong in caluclating motor commands {e}")